...
export function FacultyDashboard() {
        ...
        const fetchDashboardData = async () => {
            try {
                ...
                if (courseRes.data?.success) {
                    myCourses = courseRes.data.data.filter(
                        (c: any) =>
                            c.faculty?._id === user?._id ||
                            c.faculty === user?._id
                    );
                ...
                if (assignRes.data?.success) {
                    ...
                    myAssignments = assignRes.data.data.filter((a: any) =>
                        myCourseIds.includes(a.course?._id || a.course)
                    );
                ...
                const subResults = await Promise.allSettled(
                    myAssignments.map((a: any) =>
                        api.get(`/submissions/assignment/${a._id}`)
                    )
                );
                subResults.forEach((result) => {
                    if (
                        ...
                        pending += result.value.data.data.filter(
    ...
    if (loading)
        return (
            <Layout role="faculty">
                <div className="p-8">Loading dashboard info...</div>
            </Layout>
        );
        ...
        <Layout role="faculty">
            <div className="mb-7">
                ...
                <p className="text-sm text-gray-500 mt-1">
                    Manage your classes, assignment flow, and student activity.
                </p>
            </div>
                            ...
                            <div key={row.label}>
                                ...
                                <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
                                    <div
                                        ...
                                        style={{
                                            width: `${Math.min(100, assignments.length ? (row.value / assignments.length) * 100 : 0)}%`,
                                        }}
                                    />
                                </div>
                            </div>
